...
def ia_tendencias(servicio_top, cliente_top):
    ...
    texto = (
        "El sistema inteligente analizó el comportamiento "
        "general del taller mecánico. "
    )

    if servicio_top:
        texto += (
            f"El servicio más solicitado es "
            f"{servicio_top}. "
        )
    else:
        texto += (
            "No existen suficientes servicios registrados. "
        )

    if cliente_top:
        texto += (
            f"El cliente más frecuente es "
            f"{cliente_top}. "
        )
    else:
        texto += (
            "No existen clientes frecuentes registrados. "
        )

    texto += (
        "Se recomienda fortalecer los servicios "
        "más demandados y aplicar estrategias "
...
def ia_recomendacion(servicio_top, total_ingresos):
    ...
    texto = (
        f"La inteligencia artificial recomienda "
        f"fortalecer el servicio {servicio_top}. "
    )
    ...
    if float(total_ingresos or 0) >= 1000:
        ...
        texto += (
            ...
            "Se recomienda ampliar la capacidad operativa."
        )

    else:

        texto += (
            "Los ingresos todavía son bajos. "
            "Se recomienda aplicar promociones "
            "y descuentos estratégicos."
        )

    texto += (
        f"Predicción futura: el servicio "
        f"{servicio_top} continuará siendo "
        f"uno de los más solicitados."
    )
...
class DashboardGeneralView(BaseView):
    ...
    def index(self):
        ...
        if fecha_desde:
            ordenes_query = ordenes_query.filter(
                OrdenTrabajo.fecha_ingreso >= fecha_desde
            )

        if fecha_hasta:
            ordenes_query = ordenes_query.filter(
                OrdenTrabajo.fecha_ingreso <= fecha_hasta
            )
        ...
        total_vehiculos = db.session.query(
            func.count(Vehiculo.id)
        ).scalar() or 0
        ...
        servicios = (
            ...
            .order_by(
                func.count(DetalleServicio.id).desc()
            )
            .all()
        )
        ...
        servicio_top = (
            nombres[0]
            if nombres else "Sin datos"
        )

        analisis_ia = (
            f"El taller registra "
            f"{total_clientes} clientes, "
            f"{total_vehiculos} vehículos y "
            f"{total_ordenes} órdenes de trabajo. "
            f"Los ingresos actuales son "
            f"Bs. {float(total_ingresos):.2f}. "
            f"El servicio más solicitado es "
            f"{servicio_top}."
        )
...
class DashboardPDFView(BaseView):
    ...
    def index(self):
        ...
        pdf.drawString(
            50,
            760,
            "REPORTE INTELIGENTE DEL TALLER"
        )
        ...
        pdf.drawString(
            50,
            690,
            f"Vehículos registrados: {total_vehiculos}"
        )

        pdf.drawString(
            50,
            670,
            f"Órdenes registradas: {total_ordenes}"
        )

        pdf.drawString(
            50,
            650,
            f"Ingresos totales: Bs. {float(total_ingresos):.2f}"
        )
        ...
        return send_file(
            buffer,
            as_attachment=True,
            download_name="reporte_taller.pdf",
...
class ReporteTendenciasIAView(BaseView):
    ...
    def index(self):
        ...
        clientes = (
            ...
            .join(
                Vehiculo,
                Cliente.id == Vehiculo.cliente_id
            )
            .join(
                OrdenTrabajo,
                Vehiculo.id == OrdenTrabajo.vehiculo_id
            )
        ...
        servicio_top = (
            servicios_nombres[0]
            if servicios_nombres else None
        )

        cliente_top = (
            clientes_nombres[0]
            if clientes_nombres else None
        )

        analisis_ia = ia_tendencias(
            servicio_top,
            cliente_top
        )
...
class ReporteRecomendacionIAView(BaseView):
    ...
    def index(self):
        ...
        recomendacion_ia = ia_recomendacion(
            servicio_top,
            total_ingresos
        )
...
appbuilder.add_view(
    ...
    "Reporte 1 - General",
    icon="fa-dashboard",
    category="Reportes Inteligentes"
)

appbuilder.add_view(
    ReporteTendenciasIAView,
    "Reporte 2 - Tendencias IA",
    icon="fa-line-chart",
    category="Reportes Inteligentes"
)
